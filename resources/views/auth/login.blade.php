...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl w-full space-y-8 flex items-center justify-between">
        ...
        <div class="w-[396px]">
            @if(session('success'))
                <div class="bg-green-400 p-4 rounded-xl mb-2 text-white">
                    {{session('success')}}
                </div>
            @endif
                @if(session('error'))
                    <div class="bg-red-400 p-4 rounded-xl mb-2 text-white">
                        {{session('error')}}
                    </div>
                @endif
            <div class="bg-white p-8 rounded-xl shadow-md">
                <form class="space-y-6" action="{{route('login.proccess')}}" method="POST">
                    @csrf
                    <div>
                        <input type="email"
                               required
                    ...
                    <div class="border-t border-gray-300 pt-6">
                        <a href="{{route('register.form')}}" type="button"
